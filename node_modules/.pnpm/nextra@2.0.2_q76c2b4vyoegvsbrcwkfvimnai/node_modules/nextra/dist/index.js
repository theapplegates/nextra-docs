...
function initConfig(distDir = ".next") {
  cacheDir = import_node_path3.default.join(CWD, distDir, "cache");
  assetDir = import_node_path3.default.join(CWD, distDir, "static", "chunks");
}
function restoreCache(distDir) {
  if (!cacheDir || !assetDir) {
    initConfig(distDir);
    if (!(0, import_find_pages_dir.existsSync)(assetDir)) {
      import_graceful_fs.default.mkdirSync(assetDir, { recursive: true });
    }
    cacheDirExist = (0, import_find_pages_dir.existsSync)(cacheDir);
    if (!cacheDirExist) {
      import_graceful_fs.default.mkdirSync(cacheDir, { recursive: true });
    }
  }
  if (!cacheDirExist) {
    return;
  ...
  for (const file of files) {
    if (file.startsWith("nextra-data-")) {
      import_graceful_fs.default.copyFileSync(import_node_path3.default.join(cacheDir, file), import_node_path3.default.join(assetDir, file));
    }
  }
}
...
function collectFiles(_0) {
  return __async(this, arguments, function* (dir, route = "/", fileMap = /* @__PURE__ */ Object.create(null)) {
    ...
    const promises = files.map(
      (f) => limit(() => __async(this, null, function* () {
    ...
    const locales = new Set(
      mdxPages.filter((item) => item.kind === "MdxPage").map((item) => item.locale)
    );
...
var nextra = (...config) => function withNextra(nextConfig = {}) {
  ...
  return __spreadProps(__spreadValues({}, nextConfig), {
    ...
    webpack(config2, options) {
      ...
      return ((_d = nextConfig.webpack) == null ? void 0 : _d.call(nextConfig, config2, options)) || config2;
    }
  });
};
